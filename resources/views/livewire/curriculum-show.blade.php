...
{{--                        @endif--}}
                        @if($student->isPresentInClass($curriculum->id))
                            Presented
                        @else
                        <button wire:click="attendance({{$student->id}})" class="py-2 px-3 bg-green-500 text-white hover:bg-green-600 rounded">Present</button>
                        @endif
                    </div>
                </td>
            </tr>
